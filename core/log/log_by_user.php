...
<?php
$sql = "SELECT * FROM `view_log_by_user_date` WHERE `username` = '" . @$_REQUEST["username"] . "' ORDER BY `date` DESC ;";
  ...
  <div class="w3-col l10">
    <div class="w3-card-4 w3-animate-bottom">
      ...
      <header class="w3-container w3-theme w3-center w3-padding-32">
        <h1>ข้อมูลการประมวลผลของ <?= @$_REQUEST["username"]; ?></h1>
      </header>
        ...
        <table id="example" class="w3-table-all w3-hoverable s12 display  w3-centered" style="width:100%" border="1">
          ...
          <tbody>
            <?php foreach ($data as $key => $value) { ?>
              <tr>
                <td><?= $key + 1 ?></td>
                <td><a href="log_by_user_date.php?user=<?= $value["user"] ?>&date=<?= $value["date"] ?>&username=<?= $_REQUEST["username"]; ?>"><?= $mc->DateThai4($value["date"]); ?></a></td>
