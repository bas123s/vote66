...
// var_dump($data);
?>
<script type="text/javascript" src="https://api.longdo.com/map/?key=86292441f119142c3516a583ccef8b4e"></script>
...
<script>
  ...
  function layer() {
    ...
    foreach ($data2 as $key => $value) {
      ...
      $sql3 = "SELECT concat(`title`, ' ',`name`,' ',`surname`) as `name`, `pp_no`, `pp_name` FROM `election_district_list` WHERE pcode = '" . $value["pcode"] . "' AND election_district = '" . $value["district"] . "' AND number = '" . $number . "'";
      ...
      if (empty($person)) {
        ...
        $s = "else";
        $color = $pp_color[$person['pp_no']];
      }
      ?>
      <?php
      $title = $value['pname'] . " เขต" . $value['district'];
      $detail = "ต." . $value['tname'] . " อ." . $value['aname'] . "<table style='margin-top:5px'><tr><td><img src='../img/" . @$person['pp_name'] . ".png' style='width:90px;'></td><td style='vertical-align: text-top; margin-top:10px;'><br>พรรค : " . @$person['pp_name'] . "<br>ชื่อผู้สมัคร : " . @$person['name'] . " </td></tr></table><br><input onclick='zm(".$value[pcode].");'' type='button' value='ดูข้อมูล''>";
      ?>
...
<script type="text/javascript">function zm(val){
  window.location.assign("maps2.php?pcode="+val);
}</script>
